...
const ProductEdit = () => {
  ...
  const [base, setBase] = useState<any>({
    ...
    is_featured: false, is_new: false, sold_by_meter: false, out_of_stock: false, sort_order: 0, images: [],
  ...
  useEffect(() => {
    ...
    setBase({
      ...
      is_featured: p.is_featured || false, is_new: p.is_new || false, sold_by_meter: p.sold_by_meter || false, out_of_stock: p.out_of_stock || false,
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Product Info">
        ...
        <div className="flex items-center gap-6">
          ...
          <label className="flex items-center gap-2 text-sm text-red-600 cursor-pointer">
            <input type="checkbox" className="w-4 h-4 rounded border-red-300" checked={base.out_of_stock} onChange={(e) => setBase((p: any) => ({ ...p, out_of_stock: e.target.checked }))} />
      ...
      {base.type === "contractor" && (
        <Section title="Custom Colors (בחירת צבע מותאם אישית)">
          {/* Toggle */}
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setCustomColorsEnabled(p => !p)}
              className={`relative inline-flex w-11 h-6 rounded-full transition-colors flex-shrink-0 ${customColorsEnabled ? "bg-gray-900" : "bg-gray-300"}`}
            >
