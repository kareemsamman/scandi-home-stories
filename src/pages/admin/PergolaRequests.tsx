    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">
              {filtered.length !== requests.length
                ? `${filtered.length} מתוך ${requests.length}`
                : `${requests.length}`}{" "}
              בקשות
              {newCount > 0 && (
          ...
          <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="w-full sm:w-44 border-gray-200 text-sm h-9">
            ...
            <Select value={sortBy} onValueChange={(v) => setSortBy(v as typeof sortBy)}>
              <SelectTrigger className="w-full sm:w-36 border-gray-200 text-sm h-9">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          {q && (
            <button className="text-sm text-blue-500 hover:underline mt-1" onClick={() => setSearchQuery("")}>
              נקה חיפוש
            </button>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          {filtered.map((req) => {
            ...
            return (
              <button
                key={req.id}
                onClick={() => navigate(`/admin/pergola-requests/${req.id}`)}
              ...
              >
                ...
                <div className="shrink-0 text-center w-14">
                  <div className="text-xs font-bold text-gray-900">
                    {new Date(req.created_at).toLocaleDateString("he-IL", { day: "2-digit", month: "2-digit" })}
                  </div>
