export type Locale = 'he' | 'ar';

export const translations: Record<Locale, Record<string, any>> = {
  he: {
    common: {
      viewAll: 'הצג הכל',
      learnMore: 'למידע נוסף',
      shopNow: 'לחנות',
      contactUs: 'צרו קשר',
      currency: '₪',
      backToHome: 'חזרה לדף הבית',
    },
    nav: {
      shop: 'חנות',
      catalog: 'קטלוג',
      about: 'אודות',
      contact: 'צור קשר',
      cart: 'סל קניות',
      search: 'חיפוש',
      collections: 'קולקציות',
      account: 'חשבון',
    },
    catalog: {
      title: 'קטלוג מוצרים',
      subtitle: 'כל הקטגוריות והמוצרים שלנו במקום אחד',
    },
    announcement: {
      messages: [
        'משלוח חינם לכל הזמנה',
        'התקנה מקצועית בכל הארץ',
        'ייעוץ חינם: 052-812-2846',
      ],
    },
    hero: {
      slides: [
        { title: 'חיים בחוץ.\nבסטנדרט אחר.', subtitle: 'פרגולות ביוקלימטיות מתקדמות לחיים מושלמים בחוץ', cta: 'גלו את הקולקציה' },
        { title: 'הנדסה פוגשת\nעיצוב.', subtitle: 'מערכות למלות מוטוריות עם שליטה חכמה', cta: 'ראו את המערכות' },
        { title: 'מותאם אישית.\nבנוי להישאר.', subtitle: 'כל פרגולה מתוכננת ומיוצרת במיוחד עבורכם', cta: 'התחילו תכנון' },
      ],
    },
    categories: {
      bioclimatic: 'פרגולות ביוקלימטיות',
      motorized: 'מערכות למלות מוטוריות',
      fixed: 'פרגולות קבועות',
      retractable: 'גגות נפתחים',
      accessories: 'אביזרים ותוספות',
    },
    features: {
      title: 'למה AMG?',
      description: 'הנדסה גרמנית, ייצור ישראלי, התקנה מקצועית',
      items: [
        { title: 'אלומיניום פרימיום', description: 'חומרים עמידים לכל מזג אוויר' },
        { title: 'ניקוז מובנה', description: 'מערכת ניקוז גשם משולבת' },
        { title: 'שליטה חכמה', description: 'מנוע חשמלי או ידני' },
        { title: 'אחריות מורחבת', description: '10 שנות אחריות מלאה' },
      ],
    },
    promos: [
      { title: 'הדגמים הפופולריים', subtitle: 'הנמכרים ביותר שלנו' },
      { title: 'עמידות מקסימלית', subtitle: 'מערכות לכל מזג אוויר' },
      { title: 'ייעוץ אישי', subtitle: 'מצאו את הפרגולה המושלמת' },
    ],
    brand: {
      title: 'חיי חוץ. ללא פשרות.',
      description: 'AMG Pergola מתכננת, מייצרת ומתקינה פרגולות ומערכות הצללה מתקדמות. אנחנו מאמינים שהמרחב החיצוני שלכם ראוי לאותה רמת איכות, עיצוב והנדסה כמו הבית עצמו.',
    },
    lifestyle: {
      title: 'חוויית חיים בחוץ',
      description: 'מרחב שמשלב נוחות, עיצוב ועמידות — להנאה בכל עונה.',
      cta: 'ראו פרויקטים',
    },
    beforeAfter: {
      title: 'לפני ואחרי',
      subtitle: 'גררו את הסליידר כדי לראות את ההבדל',
      before: 'לפני',
      after: 'אחרי',
    },
    faq: {
      title: 'שאלות נפוצות',
      subtitle: 'יש שאלה? אנחנו כאן לעזור.',
      contactText: 'לא מצאתם תשובה? צרו קשר ונשמח לסייע.',
      items: [
        { question: 'כמה זמן לוקחת ההתקנה?', answer: 'התקנה סטנדרטית לוקחת בין יום אחד לשלושה ימים, בהתאם לגודל ולמורכבות הפרויקט.' },
        { question: 'מאילו חומרים הפרגולות עשויות?', answer: 'אלומיניום פרימיום בציפוי אלקטרוסטטי, עמיד לקורוזיה, UV ומזג אוויר קיצוני.' },
        { question: 'האם הפרגולות עמידות בגשם?', answer: 'בהחלט. פרגולות הלמלות המוטוריות כוללות מערכת ניקוז מובנית ואטימה מלאה.' },
        { question: 'מה כוללת האחריות?', answer: '10 שנות אחריות על המבנה ו-5 שנים על המנועים והמערכות החשמליות.' },
        { question: 'האם אתם מבצעים התקנה בכל הארץ?', answer: 'כן, אנו מבצעים התקנות בכל רחבי ישראל עם צוות מקצועי.' },
      ],
    },
    footer: {
      description: 'מומחים בפרגולות ופתרונות הצללה מתקדמים.',
      columns: [
        { title: 'קולקציות מובחרות', links: [
          { label: 'פרגולות ביוקלימטיות', href: '/shop?collection=bioclimatic' },
          { label: 'מערכות למלות', href: '/shop?collection=motorized' },
          { label: 'פרגולות קבועות', href: '/shop?collection=fixed' },
          { label: 'גגות נפתחים', href: '/shop?collection=retractable' },
          { label: 'אביזרים', href: '/shop?collection=accessories' },
        ]},
        { title: 'תמיכה', links: [
          { label: 'שאלות נפוצות', href: '/contact' },
          { label: 'אחריות', href: '/contact' },
          { label: 'משלוחים', href: '/legal/shipping' },
          { label: 'החזרות', href: '/legal/returns' },
        ]},
        { title: 'מדיניות', links: [
          { label: 'מדיניות פרטיות', href: '/legal/privacy' },
          { label: 'תנאי שימוש', href: '/legal/terms' },
        ]},
        { title: 'AMG Pergola', links: [
          { label: 'אודות', href: '/about' },
          { label: 'הסיפור שלנו', href: '/about' },
        ]},
        { title: 'יצירת קשר', links: [
          { label: 'mail@amgpergola.co.il', href: 'mailto:mail@amgpergola.co.il' },
          { label: '052-812-2846', href: 'tel:+972528122846' },
        ]},
      ],
      copyright: '© {year}, AMG Pergola',
    },
    product: {
      addToBag: 'הוסף לסל',
      addToWishlist: 'שמור למועדפים',
      savedToWishlist: 'נשמר במועדפים',
      quantity: 'כמות',
      materials: 'חומרים',
      dimensions: 'מידות',
      shipping: 'משלוח',
      shippingText: 'משלוח חינם',
      returns: 'החזרות',
      returnsText: '14 ימי החזרה',
      warranty: 'אחריות',
      warrantyText: '10 שנים',
      relatedProducts: 'מוצרים דומים',
      moreFrom: 'עוד מ',
      viewDetails: 'לפרטים',
      newBadge: 'חדש',
      featuredBadge: 'מומלץ',
      pieces: 'מוצרים',
      piece: 'מוצר',
      notFound: 'המוצר לא נמצא',
      notFoundText: 'המוצר שחיפשת לא קיים.',
      browseAll: 'לכל המוצרים',
      noProducts: 'לא נמצאו מוצרים',
      noProductsText: 'קולקציה זו בהכנה.',
      zoomImage: 'הגדל תמונה',
      specifications: 'מפרט טכני',
      features: 'תכונות',
      description: 'תיאור',
      about: 'אודות',
      aboutProduct: 'אודות המוצר',
      productDetails: 'פרטי המוצר',
      adding: 'מוסיף...',
      reviews: 'ביקורות',
      noReviews: 'אין ביקורות עדיין',
      writeReview: 'כתוב ביקורת',
    },
    cart: {
      title: 'סל הקניות',
      empty: 'הסל ריק',
      emptyText: 'גלו את מגוון הפרגולות שלנו.',
      startShopping: 'לחנות',
      continueShopping: 'המשך קניות',
      subtotal: 'סכום ביניים',
      shipping: 'משלוח',
      complimentary: 'חינם',
      freeShippingNote: 'משלוח חינם',
      freeShippingEligible: 'משלוח חינם',
      total: 'סה"כ',
      checkout: 'לתשלום',
      orderSummary: 'סיכום הזמנה',
      product: 'מוצר',
      quantity: 'כמות',
      totalCol: 'סה"כ',
      remove: 'הסרה',
      estimateShipping: 'חישוב עלות משלוח',
      taxNote: 'מיסים יחושבו בתשלום',
      vatLabel: 'מע"מ',
      includesVat: 'כולל מע"מ',
      freeShippingUnlocked: 'משלוח חינם!',
      freeShippingRemaining: 'עוד {amount} למשלוח חינם',
    },
    checkout: {
      title: 'השלמת הזמנה',
      contactInfo: 'פרטי קשר',
      firstName: 'שם פרטי',
      lastName: 'שם משפחה',
      email: 'דוא"ל',
      phone: 'טלפון',
      shippingAddress: 'כתובת למשלוח',
      address: 'כתובת',
      addressPlaceholder: 'הקלד כתובת...',
      apartment: 'דירה / קומה (אופציונלי)',
      apartmentPlaceholder: 'דירה, קומה, כניסה...',
      shippingMethod: 'שיטת משלוח',
      freeDelivery: 'משלוח חינם',
      deliveryTime: '5–8 ימי עסקים',
      paymentMethod: 'אמצעי תשלום',
      bankTransfer: 'העברה בנקאית',
      bankTransferNote: 'לאחר ביצוע ההזמנה תקבלו הוראות להעברה בנקאית במייל. ההזמנה תטופל לאחר אישור התשלום.',
      selected: 'נבחר',
      orderNote: 'הערה להזמנה',
      orderNotePlaceholder: 'הוסיפו הערה מיוחדת להזמנה (אופציונלי)',
      placeOrder: 'ביצוע הזמנה',
      submitting: 'שולח...',
      successTitle: 'ההזמנה התקבלה',
      successText: 'תודה! תקבלו מייל עם הוראות תשלום בקרוב.',
      required: 'שדה חובה',
      firstNameRequired: 'שם פרטי חובה',
      lastNameRequired: 'שם משפחה חובה',
      emailRequired: 'דוא"ל חובה',
      invalidEmail: 'כתובת דוא"ל לא תקינה',
      phoneRequired: 'מספר טלפון חובה',
      invalidPhone: 'מספר טלפון ישראלי לא תקין (לדוגמה: 050-1234567)',
      addressRequired: 'כתובת חובה',
      showSummary: 'סיכום הזמנה',
      emptyTitle: 'אין פריטים',
      emptyText: 'הסל ריק.',
      city: 'עיר',
      street: 'רחוב',
      houseNumber: 'מספר',
      chooseCity: 'בחר עיר',
      selectCity: 'יש לבחור עיר מהרשימה',
      discountPlaceholder: 'קוד הנחה או כרטיס מתנה',
      applyDiscount: 'החל',
      invalidDiscount: 'הזן קוד הנחה או כרטיס מתנה תקין',
      discount: 'הנחה',
      discountAppliedLabel: 'קוד הנחה:',
      refundPolicy: 'מדיניות החזרות',
      shippingPolicy: 'משלוחים',
      privacyPolicy: 'מדיניות פרטיות',
      termsOfService: 'תנאי שימוש',
      emailMarketing: 'שלח לי עדכונים, מבצעים והטבות בדוא״ל',
      acceptPrivacy: 'אני מאשר שקראתי ואני מסכים ל',
      loginLink: 'התחברות',
      newAddress: 'כתובת חדשה',
      shippingZone: 'אזור משלוח',
      shippingZonePlaceholder: 'בחרו אזור',
      shippingZoneNote: 'הזינו עיר תחילה',
      shippingFreeAbove: 'משלוח חינם בהזמנות מעל ₪{amount}',
      shippingZoneNames: {
        north: 'צפון',
        center: 'מרכז',
        south: 'דרום',
        jerusalem: 'ירושלים',
      },
    },
    payment: {
      title: 'השלמת תשלום',
      instructions: 'כדי להשלים את ההזמנה יש לבצע העברה בנקאית לפרטי הבנק המופיעים למטה.\nלאחר ביצוע ההעברה יש להעלות צילום של אישור ההעברה.\nלאחר אימות האישור ההזמנה תאושר.',
      bankDetailsTitle: 'פרטי העברה בנקאית',
      bankName: 'שם הבנק',
      accountName: 'שם החשבון',
      accountNumber: 'מספר חשבון',
      branchNumber: 'מספר סניף',
      bankCode: 'קוד בנק',
      amountToPay: 'סכום לתשלום',
      uploadTitle: 'העלאת אישור העברה',
      dropText: 'גרור לכאן את אישור ההעברה\nאו לחץ להעלאת קובץ',
      receiptDetected: 'נראה כי הקובץ מכיל אישור העברה.',
      submitReceipt: 'שלח אישור תשלום',
      backToForm: 'חזרה לטופס',
    },
    thankYou: {
      title: '✔ תודה על ההזמנה!',
      receiptReceived: 'קיבלנו את אישור התשלום שלך.',
      underReview: 'ההזמנה שלך נמצאת כעת בבדיקה. נעדכן אותך לאחר אימות ההעברה הבנקאית.',
      orderNumber: 'מספר הזמנה',
      orderDate: 'תאריך הזמנה',
      status: 'סטטוס',
      pendingVerification: 'ממתין לאישור תשלום',
      backToHome: 'חזרה לדף הבית',
      viewOrder: 'צפייה בהזמנה',
    },
    auth: {
      login: 'התחברות',
      signup: 'הרשמה',
      identifier: 'דוא"ל / שם משתמש / טלפון',
      password: 'סיסמה',
      forgotPassword: 'שכחת סיסמה?',
      forgotPwTitle: 'שחזור סיסמה',
      forgotPwDesc: 'הזינו את כתובת המייל שלכם ונשלח לכם קישור לאיפוס הסיסמה.',
      forgotPwSent: 'שלחנו לכם מייל עם קישור לאיפוס הסיסמה. בדקו את תיבת הדואר.',
      forgotPwError: 'שגיאה',
      forgotPwErrorText: 'לא הצלחנו לשלוח מייל לאיפוס סיסמה. נסו שוב.',
      emailLabel: 'דוא"ל',
      sendResetLink: 'שליחת קישור איפוס',
      backToLogin: 'חזרה להתחברות',
      resetPwTitle: 'איפוס סיסמה',
      newPassword: 'סיסמה חדשה',
      confirmPassword: 'אימות סיסמה',
      pwMinLength: 'הסיסמה חייבת להכיל לפחות 6 תווים',
      pwMismatch: 'הסיסמאות אינן תואמות',
      resetPwError: 'שגיאה באיפוס הסיסמה, נסו שוב',
      resetPwSuccess: 'הסיסמה שונתה בהצלחה! מעבירים אתכם להתחברות...',
      resetPwButton: 'עדכון סיסמה',
      noAccount: 'אין לך חשבון?',
      signupLink: 'הרשמה',
      hasAccount: 'יש לך כבר חשבון?',
      loginLink: 'התחברות',
      fullName: 'שם מלא',
      username: 'שם משתמש',
      createAccount: 'יצירת חשבון',
      loginSuccess: 'התחברת בהצלחה',
      loginSuccessText: 'ברוך הבא!',
      signupSuccess: 'החשבון נוצר בהצלחה',
      signupSuccessText: 'ברוך הבא! תוכל עכשיו להתחבר.',
      logout: 'התנתקות',
    },
    account: {
      orders: 'הזמנות',
      addresses: 'כתובות',
      profile: 'פרופיל',
      noOrders: 'עדיין לא ביצעת הזמנות',
      noOrdersText: 'כשתבצע הזמנה, היא תופיע כאן.',
      orderNumber: 'הזמנה',
      statusPending: 'ממתין לאישור תשלום',
      statusConfirmed: 'אושרה',
      statusShipped: 'נשלחה',
      statusDelivered: 'נמסרה',
      statusWaitingApproval: 'מחכה אישור',
      statusInProcess: 'בתהליך',
      statusInDelivery: 'יצא למשלוח',
      statusNotApproved: 'לא אושרה',
      statusCancelled: 'בוטלה',
      savedAddresses: 'כתובות שמורות',
      addAddress: 'הוסף כתובת',
      noAddresses: 'אין כתובות שמורות',
      defaultAddress: 'ברירת מחדל',
      edit: 'עריכה',
      delete: 'מחיקה',
      street: 'רחוב',
      houseNumber: 'מספר בית',
      saveAddress: 'שמור כתובת',
      cancel: 'ביטול',
      personalInfo: 'מידע אישי',
      saveChanges: 'שמור שינויים',
      saved: 'נשמר',
      saveError: 'שגיאה בשמירה, נסה שוב',
      changePassword: 'שינוי סיסמה',
      newPassword: 'סיסמה חדשה',
      confirmPassword: 'אימות סיסמה',
      updatePassword: 'עדכן סיסמה',
      passwordRequired: 'נדרשת סיסמה',
      passwordTooShort: 'לפחות 6 תווים',
      passwordMismatch: 'הסיסמאות אינן תואמות',
      selectAddress: 'בחר כתובת',
      cityRequired: 'עיר חובה',
      streetRequired: 'רחוב חובה',
      houseNumberRequired: 'מספר בית חובה',
      username: 'שם משתמש',
      setDefault: 'קבע כברירת מחדל',
      orderDetails: 'פרטי הזמנה',
    },
    about: {
      heroTitle: 'הנדסת חוץ.\nברמה אחרת.',
      heroSubtitle: 'אודות AMG',
      heroDescription: 'מתכננים, מייצרים ומתקינים פרגולות ומערכות הצללה מתקדמות מ-2008.',
      missionTitle: 'אנחנו מאמינים שהמרחב החיצוני שלכם ראוי לאותה רמת איכות כמו הבית עצמו.',
      storyTitle: 'הסיפור שלנו',
      storyText: 'AMG Pergola נוסדה מתוך תשוקה להנדסה מדויקת ועיצוב ללא פשרות. מאז 2008, אנו מובילים את שוק הפרגולות בישראל.',
      storyText2: 'כל פרגולה עוברת תכנון קפדני, ייצור מדויק והתקנה מקצועית.',
      valuesTitle: 'הערכים שלנו',
      values: [
        { title: 'הנדסת דיוק', number: '01', description: 'כל פרגולה מתוכננת בדייקנות מירבית, עם חומרים שנבחרו לעמידות.' },
        { title: 'חדשנות', number: '02', description: 'מערכות חכמות, מנועים שקטים ושליטה מרחוק.' },
        { title: 'שירות מקצועי', number: '03', description: 'מהייעוץ הראשוני ועד ההתקנה — צוות מומחים מלווה אתכם.' },
      ],
      ctaTitle: 'מוכנים להתחיל?',
      ctaText: 'צרו קשר לייעוץ חינם.',
      ctaButton: 'דברו איתנו',
    },
    contact: {
      title: 'צרו קשר',
      subtitle: 'נשמח לשמוע מכם.',
      name: 'שם מלא',
      email: 'דוא"ל',
      phone: 'טלפון',
      message: 'הודעה',
      messagePlaceholder: 'ספרו לנו על הפרויקט שלכם...',
      send: 'שלח הודעה',
      sending: 'שולח...',
      infoTitle: 'פרטי התקשרות',
      emailLabel: 'דוא"ל',
      phoneLabel: 'טלפון',
      addressLabel: 'כתובת',
      addressText: 'עצמונה 1, מעלה אדומים',
      hoursLabel: 'שעות פעילות',
      hoursText: 'א׳-ה׳ 08:00-17:00',
    },
    shop: {
      title: 'כל המוצרים',
      sortBy: 'מיין לפי',
      sortOptions: [
        { value: 'featured', label: 'מומלצים' },
        { value: 'newest', label: 'חדשים' },
        { value: 'price-asc', label: 'מחיר: נמוך לגבוה' },
        { value: 'price-desc', label: 'מחיר: גבוה לנמוך' },
        { value: 'name-asc', label: 'א-ת' },
      ],
      filterAll: 'הכל',
      needHelp: 'צריכים עזרה?',
      helpText: 'אנחנו כאן בשבילכם',
      filters: {
        title: 'סינון',
        searchProducts: 'חיפוש מוצרים',
        searchPlaceholder: 'חפש לפי שם או מק״ט...',
        category: 'קטגוריה',
        subCategory: 'תת קטגוריה',
        brand: 'מותג',
        priceRange: 'מחיר',
        priceMin: '₪ מינימום',
        priceMax: '₪ מקסימום',
        length: 'אורך',
        color: 'צבע',
        sku: 'מק״ט',
        skuPlaceholder: 'חפש לפי מק״ט...',
        sort: 'מיון',
        newest: 'חדש ביותר',
        clearAll: 'נקה הכל',
        showResults: 'הצג {count} מוצרים',
        categories: 'קטגוריות',
        meter: 'מטר',
      },
    },
    contractor: {
      sku: 'מק״ט',
      length: 'אורך',
      size: 'אורך',
      color: 'צבע',
      selectedColor: 'צבע נבחר',
      selectSize: 'יש לבחור אורך',
      selectColor: 'יש לבחור צבע',
      customColor: 'בחר צבע מותאם',
      customColorTitle: 'בחר צבע לשינוי',
      searchColor: 'חפש לפי שם או קוד RAL...',
      cancel: 'ביטול',
      confirmSelection: 'אישור בחירה',
      meter: 'מטר',
      addedToCart: 'נוסף לסל',
      colorTabs: {
        ral: 'RAL צבע קוד',
        passivation: 'פסיבציה',
        noColorMF: 'ללא צבע MF',
        woodLook: 'דמוי עץ',
        iron: 'אירון',
        metal: 'מטאל',
      },
    },
    miniCart: {
      empty: 'הסל שלך ריק',
      continueShopping: 'המשך קניות',
      freeShipping: 'אתם זכאים למשלוח חינם.',
      remove: 'הסר',
      buyWith: 'קנו יחד עם',
      add: 'הוסף',
      taxNote: 'מיסים ומשלוח יחושבו בתשלום',
      viewCart: 'צפייה בסל',
    },
    search: {
      placeholder: 'חיפוש...',
      clear: 'נקה',
      featuredCollections: 'קולקציות מובחרות',
      noResults: 'לא נמצאו תוצאות.',
      tabs: {
        products: 'מוצרים',
        suggestions: 'הצעות',
        collections: 'קולקציות',
        blog: 'בלוג',
      },
    },
    notFound: {
      title: 'הדף לא נמצא',
      text: 'הדף שחיפשתם לא קיים.',
      home: 'חזרה לדף הבית',
      shop: 'לחנות',
    },
    pergolaRequest: {
      pageTitle: 'הגשת בקשה לפרגולה',
      pageSubtitle: 'מלאו את הפרטים וקבלו הצעה מותאמת',
      dimensions: 'מידות',
      width: 'רוחב (ס"מ)',
      length: 'אורך (ס"מ)',
      height: 'גובה (ס"מ) - אופציונלי',
      configuration: 'תצורה',
      pergolaType: 'סוג פרגולה',
      pergolaTypes: {
        fixed: 'פרגולה קבועה',
        pvc: 'פרגולה PVC',
      },
      mountType: 'סוג התקנה',
      mountWall: 'צמוד קיר',
      mountFreestanding: 'עצמאי',
      installation: 'התקנה מקצועית',
      lighting: 'תאורה',
      lightingNone: 'ללא',
      lightingWhite: 'לבנה',
      lightingRgb: 'RGB',
      lightingPosition: 'מיקום תאורה',
      lightingAllPosts: 'כל העמודים',
      lightingSelectedPosts: 'עמודים נבחרים',
      lightingNoPosts: 'ללא עמודים',
      lightingFixture: 'סוג גוף תאורה',
      fixtureSpotlight: 'ספוטלייט',
      fixtureLedStrip: 'פס LED',
      fixtureRgbStrip: 'פס RGB',
      fixtureMixed: 'חבילה מעורבת',
      lightingRoof: 'תאורת גג / היקף',
      lightingLabel: 'תאורה',
      santafRoofing: 'גג סנטף',
      santafWithout: 'ללא סנטף',
      santafWith: 'עם סנטף',
      santafColorLabel: 'צבע סנטף',
      colors: 'צבעים',
      frameColor: 'צבע מסגרת',
      roofColor: 'צבע גג / בד',
      spacing: 'מרווח בין פרופילים',
      spacingAutomatic: 'אוטומטי',
      spacingDense: 'צפוף',
      spacingStandard: 'רגיל',
      spacingWide: 'רחב',
      spacingLabel: 'מרווח',
      notes: 'הערות',
      notesPlaceholder: 'ספרו לנו על הצרכים שלכם...',
      customerInfo: 'פרטי לקוח',
      customerName: 'שם מלא',
      customerPhone: 'טלפון',
      customerEmail: 'דוא"ל (אופציונלי)',
      specs: 'מפרט טכני',
      moduleType: 'מודול',
      moduleSingle: 'יחיד',
      moduleDouble: 'כפול',
      moduleTriple: 'משולש',
      moduleCustom: 'דורש בדיקה',
      carriers: 'קורות חלוקה',
      posts: 'עמודים',
      frontPosts: 'עמודים קדמיים',
      backPosts: 'עמודים אחוריים',
      profileRafter: 'פרופיל רפטר',
      profileCarrierPost: 'פרופיל עמוד קורת חלוקה',
      profileFabricMaster: 'פרופיל בד ראשי',
      preview: 'תצוגה מקדימה',
      topView: 'מבט עליון',
      frontView: 'מבט קדמי',
      isometricView: 'תלת מימד',
      submit: 'שלח בקשה',
      submitting: 'שולח...',
      submitConfirmTitle: 'אישור שליחת בקשה',
      submitConfirmText: 'האם אתם בטוחים שברצונכם לשלוח את הבקשה?',
      confirm: 'אישור',
      cancel: 'ביטול',
      successTitle: 'הבקשה נשלחה בהצלחה!',
      successText: 'ניצור איתכם קשר בהקדם עם הצעת מחיר.',
      backToHome: 'חזרה לדף הבית',
      disclaimer: 'זהו סיכום בקשה, לא הצעת מחיר סופית. השרטוט ופיזור העמודים הם ראשוניים ומותנים בבדיקה טכנית ובביקור באתר.',
      customReviewNote: 'הרוחב שהוזן דורש בדיקת התאמה מיוחדת. נציג ייצור איתכם קשר.',
      required: 'שדה חובה',
      invalidPhone: 'מספר טלפון לא תקין',
      minWidth: 'רוחב מינימלי 100 ס"מ',
      maxWidth: 'רוחב מקסימלי 1500 ס"מ',
      minLength: 'אורך מינימלי 200 ס"מ',
      maxLength: 'אורך מקסימלי 1000 ס"מ',
      yes: 'כן',
      no: 'לא',
      clickToEdit: 'לחצו על רכיב בשרטוט לעריכה',
      startSubtitle: 'הזינו מידות בסיסיות ובחרו סוג פרגולה כדי להתחיל',
      startEditor: 'התחל עיצוב',
      nextStep: 'המשך',
      backToEditor: 'חזרה לעורך',
      summaryTitle: 'סיכום טכני',
      partsTitle: 'רכיבים ופרופילים',
      partsSubtitle: 'הרכיבים המרכיבים את הפרגולה שלכם',
      roofFillMode: 'מצב מילוי גג',
      roofSlats: 'שלבים / פרופילים',
      roofSantafOnly: 'סנטף בלבד',
      slatsLabel: 'שלבים',
      slatGap: 'מרווח בין שלבים',
      slatColor: 'צבע שלבים',
      slatSizeLabel: 'גודל פרופיל שלב',
      slatsPerCarrier: 'שלבים לכל קורת חלוקה',
      globalApplyHint: 'שינוי כאן חל על כל קורות החלוקה. לחצו על קורת חלוקה ספציפית בשרטוט להתאמה אישית',
      lightingPosts: 'תאורת עמודים',
      lightingLengthLabel: 'אורך פס תאורה',
      lightingPerCarrierHint: 'לחצו על קורת חלוקה בשרטוט לבחירת תאורה לכל קורת חלוקה בנפרד',
      phoneMustBe10: 'מספר טלפון חייב להכיל 10 ספרות',
      downloadPdf: 'הורד PDF',
      downloadPdfTitle: 'סיכום הבקשה מוכן',
      downloadPdfDesc: 'כולל שרטוטים טכניים, מפרט ופרטי תצורה',
    },
  },
  ar: {
    common: {
      viewAll: 'عرض الكل',
      learnMore: 'اعرف المزيد',
      shopNow: 'تسوق الآن',
      contactUs: 'اتصل بنا',
      currency: '₪',
      backToHome: 'العودة للرئيسية',
    },
    nav: {
      shop: 'المتجر',
      catalog: 'الكتالوج',
      about: 'من نحن',
      contact: 'اتصل بنا',
      cart: 'سلة التسوق',
      search: 'بحث',
      collections: 'المجموعات',
      account: 'حساب',
    },
    catalog: {
      title: 'كتالوج المنتجات',
      subtitle: 'جميع الفئات والمنتجات في مكان واحد',
    },
    announcement: {
      messages: [
        'شحن مجاني لجميع الطلبات',
        'تركيب احترافي في جميع أنحاء البلاد',
        'استشارة مجانية: 052-812-2846',
      ],
    },
    hero: {
      slides: [
        { title: 'حياة في الهواء الطلق.\nبمعايير مختلفة.', subtitle: 'برجولات بيوكليماتيكية متقدمة لحياة مثالية في الخارج', cta: 'اكتشف المجموعة' },
        { title: 'الهندسة تلتقي\nبالتصميم.', subtitle: 'أنظمة شرائح آلية مع تحكم ذكي', cta: 'شاهد الأنظمة' },
        { title: 'مصمم خصيصاً.\nمبني ليبقى.', subtitle: 'كل برجولة مصممة ومصنعة خصيصاً لكم', cta: 'ابدأ التخطيط' },
      ],
    },
    categories: {
      bioclimatic: 'برجولات بيوكليماتيكية',
      motorized: 'أنظمة شرائح آلية',
      fixed: 'برجولات ثابتة',
      retractable: 'أسقف قابلة للطي',
      accessories: 'إكسسوارات وإضافات',
    },
    features: {
      title: 'لماذا AMG؟',
      description: 'هندسة ألمانية، تصنيع إسرائيلي، تركيب احترافي',
      items: [
        { title: 'ألمنيوم فاخر', description: 'مواد متينة لجميع الأحوال الجوية' },
        { title: 'تصريف مدمج', description: 'نظام تصريف مياه الأمطار متكامل' },
        { title: 'تحكم ذكي', description: 'محرك كهربائي أو يدوي' },
        { title: 'ضمان ممتد', description: '10 سنوات ضمان شامل' },
      ],
    },
    promos: [
      { title: 'النماذج الأكثر شعبية', subtitle: 'الأكثر مبيعاً لدينا' },
      { title: 'متانة قصوى', subtitle: 'أنظمة لجميع الأحوال الجوية' },
      { title: 'استشارة شخصية', subtitle: 'اعثر على البرجولة المثالية' },
    ],
    brand: {
      title: 'حياة في الهواء الطلق. بدون تنازلات.',
      description: 'AMG Pergola تصمم وتصنع وتركب برجولات وأنظمة تظليل متقدمة. نؤمن بأن مساحتكم الخارجية تستحق نفس مستوى الجودة والتصميم.',
    },
    lifestyle: {
      title: 'تجربة الحياة في الهواء الطلق',
      description: 'مساحة تجمع بين الراحة والتصميم والمتانة.',
      cta: 'شاهد المشاريع',
    },
    beforeAfter: {
      title: 'قبل وبعد',
      subtitle: 'اسحب المقبض لرؤية الفرق',
      before: 'قبل',
      after: 'بعد',
    },
    faq: {
      title: 'الأسئلة الشائعة',
      subtitle: 'لديك سؤال؟ نحن هنا للمساعدة.',
      contactText: 'لم تجد إجابة؟ تواصل معنا.',
      items: [
        { question: 'كم يستغرق التركيب؟', answer: 'التركيب القياسي يستغرق من يوم إلى ثلاثة أيام، حسب الحجم والتعقيد.' },
        { question: 'من أي مواد تصنع البرجولات؟', answer: 'ألمنيوم فاخر بطلاء إلكتروستاتيكي، مقاوم للتآكل والأشعة فوق البنفسجية.' },
        { question: 'هل البرجولات مقاومة للمطر؟', answer: 'بالتأكيد. تشمل نظام تصريف مدمج وعزل كامل.' },
        { question: 'ماذا يشمل الضمان؟', answer: '10 سنوات على الهيكل و5 سنوات على المحركات والأنظمة الكهربائية.' },
        { question: 'هل تقومون بالتركيب في جميع أنحاء البلاد؟', answer: 'نعم، في جميع أنحاء إسرائيل مع فريق متخصص.' },
      ],
    },
    footer: {
      description: 'خبراء في البرجولات وحلول التظليل المتقدمة.',
      columns: [
        { title: 'مجموعات مميزة', links: [
          { label: 'برجولات بيوكليماتيكية', href: '/shop?collection=bioclimatic' },
          { label: 'أنظمة شرائح', href: '/shop?collection=motorized' },
          { label: 'برجولات ثابتة', href: '/shop?collection=fixed' },
          { label: 'أسقف قابلة للطي', href: '/shop?collection=retractable' },
          { label: 'إكسسوارات', href: '/shop?collection=accessories' },
        ]},
        { title: 'الدعم', links: [
          { label: 'الأسئلة الشائعة', href: '/contact' },
          { label: 'الضمان', href: '/contact' },
          { label: 'الشحن', href: '/legal/shipping' },
          { label: 'الإرجاع', href: '/legal/returns' },
        ]},
        { title: 'السياسات', links: [
          { label: 'سياسة الخصوصية', href: '/legal/privacy' },
          { label: 'شروط الاستخدام', href: '/legal/terms' },
        ]},
        { title: 'AMG Pergola', links: [
          { label: 'من نحن', href: '/about' },
          { label: 'قصتنا', href: '/about' },
        ]},
        { title: 'التواصل', links: [
          { label: 'mail@amgpergola.co.il', href: 'mailto:mail@amgpergola.co.il' },
          { label: '052-812-2846', href: 'tel:+972528122846' },
        ]},
      ],
      copyright: '© {year}, AMG Pergola',
    },
    product: {
      addToBag: 'أضف إلى السلة',
      addToWishlist: 'أضف للمفضلة',
      savedToWishlist: 'محفوظ في المفضلة',
      quantity: 'الكمية',
      materials: 'المواد',
      dimensions: 'الأبعاد',
      shipping: 'الشحن',
      shippingText: 'شحن مجاني',
      returns: 'الإرجاع',
      returnsText: '14 يوم للإرجاع',
      warranty: 'الضمان',
      warrantyText: '10 سنوات',
      relatedProducts: 'منتجات مشابهة',
      moreFrom: 'المزيد من',
      viewDetails: 'عرض التفاصيل',
      newBadge: 'جديد',
      featuredBadge: 'مميز',
      pieces: 'منتجات',
      piece: 'منتج',
      notFound: 'المنتج غير موجود',
      notFoundText: 'المنتج الذي تبحث عنه غير موجود.',
      browseAll: 'جميع المنتجات',
      noProducts: 'لا توجد منتجات',
      noProductsText: 'هذه المجموعة قيد التحضير.',
      zoomImage: 'تكبير الصورة',
      specifications: 'المواصفات التقنية',
      features: 'الميزات',
      description: 'الوصف',
      about: 'حول',
      aboutProduct: 'حول المنتج',
      productDetails: 'تفاصيل المنتج',
      adding: 'جاري الإضافة...',
      reviews: 'التقييمات',
      noReviews: 'لا توجد تقييمات بعد',
      writeReview: 'اكتب تقييم',
    },
    cart: {
      title: 'سلة التسوق',
      empty: 'السلة فارغة',
      emptyText: 'اكتشف مجموعتنا من البرجولات.',
      startShopping: 'تسوق الآن',
      continueShopping: 'متابعة التسوق',
      subtotal: 'المجموع الفرعي',
      shipping: 'الشحن',
      complimentary: 'مجاني',
      freeShippingNote: 'شحن مجاني',
      freeShippingEligible: 'شحن مجاني',
      total: 'الإجمالي',
      checkout: 'إتمام الشراء',
      orderSummary: 'ملخص الطلب',
      product: 'المنتج',
      quantity: 'الكمية',
      totalCol: 'الإجمالي',
      remove: 'إزالة',
      estimateShipping: 'حساب تكلفة الشحن',
      taxNote: 'الضرائب تُحسب عند الدفع',
      vatLabel: 'ض.ق.م',
      includesVat: 'شامل ضريبة القيمة المضافة',
      freeShippingUnlocked: 'شحن مجاني!',
      freeShippingRemaining: 'باقي {amount} للشحن المجاني',
    },
    checkout: {
      title: 'إتمام الطلب',
      contactInfo: 'معلومات الاتصال',
      firstName: 'الاسم الأول',
      lastName: 'اسم العائلة',
      email: 'البريد الإلكتروني',
      phone: 'الهاتف',
      shippingAddress: 'عنوان الشحن',
      address: 'العنوان',
      addressPlaceholder: 'اكتب العنوان...',
      apartment: 'شقة / طابق (اختياري)',
      apartmentPlaceholder: 'شقة، طابق، مدخل...',
      shippingMethod: 'طريقة الشحن',
      freeDelivery: 'شحن مجاني',
      deliveryTime: '5–8 أيام عمل',
      paymentMethod: 'طريقة الدفع',
      bankTransfer: 'تحويل بنكي',
      bankTransferNote: 'بعد تقديم الطلب ستتلقى تعليمات التحويل البنكي عبر البريد الإلكتروني. سيتم معالجة طلبك بعد تأكيد الدفع.',
      selected: 'محدد',
      orderNote: 'ملاحظة للطلب',
      orderNotePlaceholder: 'أضف ملاحظة خاصة للطلب (اختياري)',
      placeOrder: 'تأكيد الطلب',
      submitting: 'جاري الإرسال...',
      successTitle: 'تم استلام الطلب',
      successText: 'شكراً! ستتلقى بريداً إلكترونياً بتعليمات الدفع قريباً.',
      required: 'حقل مطلوب',
      firstNameRequired: 'الاسم الأول مطلوب',
      lastNameRequired: 'اسم العائلة مطلوب',
      emailRequired: 'البريد الإلكتروني مطلوب',
      invalidEmail: 'بريد إلكتروني غير صالح',
      phoneRequired: 'رقم الهاتف مطلوب',
      invalidPhone: 'رقم هاتف إسرائيلي غير صالح (مثال: 050-1234567)',
      addressRequired: 'العنوان مطلوب',
      showSummary: 'ملخص الطلب',
      emptyTitle: 'لا توجد عناصر',
      emptyText: 'السلة فارغة.',
      city: 'المدينة',
      street: 'الشارع',
      houseNumber: 'الرقم',
      chooseCity: 'اختر مدينة',
      selectCity: 'يجب اختيار مدينة من القائمة',
      discountPlaceholder: 'رمز الخصم أو بطاقة هدية',
      applyDiscount: 'تطبيق',
      invalidDiscount: 'أدخل رمز خصم أو بطاقة هدية صالحة',
      discount: 'خصم',
      discountAppliedLabel: 'رمز الخصم:',
      refundPolicy: 'سياسة الاسترداد',
      shippingPolicy: 'الشحن',
      privacyPolicy: 'سياسة الخصوصية',
      termsOfService: 'شروط الخدمة',
      emailMarketing: 'أرسل لي تحديثات وعروض وخصومات عبر البريد الإلكتروني',
      acceptPrivacy: 'أقر بأنني قرأت وأوافق على',
      loginLink: 'تسجيل الدخول',
      newAddress: 'عنوان جديد',
      shippingZone: 'منطقة الشحن',
      shippingZonePlaceholder: 'اختر المنطقة',
      shippingZoneNote: 'أدخل المدينة أولاً',
      shippingFreeAbove: 'شحن مجاني للطلبات فوق ₪{amount}',
      shippingZoneNames: {
        north: 'شمال',
        center: 'مركاز',
        south: 'جنوب',
        jerusalem: 'القدس',
      },
    },
    payment: {
      title: 'إتمام الدفع',
      instructions: 'لإتمام الطلب يرجى إجراء تحويل بنكي إلى تفاصيل البنك أدناه.\nبعد إجراء التحويل يرجى رفع صورة من إيصال التحويل.\nسيتم تأكيد الطلب بعد التحقق من الإيصال.',
      bankDetailsTitle: 'تفاصيل التحويل البنكي',
      bankName: 'اسم البنك',
      accountName: 'اسم الحساب',
      accountNumber: 'رقم الحساب',
      branchNumber: 'رقم الفرع',
      bankCode: 'رمز البنك',
      amountToPay: 'المبلغ المطلوب',
      uploadTitle: 'رفع إيصال التحويل',
      dropText: 'اسحب إيصال التحويل هنا\nأو اضغط لرفع ملف',
      receiptDetected: 'يبدو أن الملف يحتوي على إيصال تحويل.',
      submitReceipt: 'إرسال إيصال الدفع',
      backToForm: 'العودة للنموذج',
    },
    thankYou: {
      title: '✔ شكراً على طلبك!',
      receiptReceived: 'تم استلام إيصال الدفع الخاص بك.',
      underReview: 'طلبك قيد المراجعة حالياً. سنُبلغك بعد التحقق من التحويل البنكي.',
      orderNumber: 'رقم الطلب',
      orderDate: 'تاريخ الطلب',
      status: 'الحالة',
      pendingVerification: 'في انتظار تأكيد الدفع',
      backToHome: 'العودة للرئيسية',
      viewOrder: 'عرض الطلب',
    },
    auth: {
      login: 'تسجيل الدخول',
      signup: 'إنشاء حساب',
      identifier: 'البريد الإلكتروني / اسم المستخدم / الهاتف',
      password: 'كلمة المرور',
      forgotPassword: 'نسيت كلمة المرور؟',
      forgotPwTitle: 'استعادة كلمة المرور',
      forgotPwDesc: 'أدخل بريدك الإلكتروني وسنرسل لك رابط إعادة تعيين كلمة المرور.',
      forgotPwSent: 'أرسلنا لك بريدًا إلكترونيًا يحتوي على رابط إعادة التعيين. تحقق من صندوق الوارد.',
      forgotPwError: 'خطأ',
      forgotPwErrorText: 'تعذر إرسال بريد إعادة تعيين كلمة المرور. حاول مرة أخرى.',
      emailLabel: 'البريد الإلكتروني',
      sendResetLink: 'إرسال رابط الإعادة',
      backToLogin: 'العودة لتسجيل الدخول',
      resetPwTitle: 'إعادة تعيين كلمة المرور',
      newPassword: 'كلمة المرور الجديدة',
      confirmPassword: 'تأكيد كلمة المرور',
      pwMinLength: 'يجب أن تحتوي كلمة المرور على 6 أحرف على الأقل',
      pwMismatch: 'كلمات المرور غير متطابقة',
      resetPwError: 'خطأ في إعادة تعيين كلمة المرور، حاول مرة أخرى',
      resetPwSuccess: 'تم تغيير كلمة المرور بنجاح! جارٍ التحويل لتسجيل الدخول...',
      resetPwButton: 'تحديث كلمة المرور',
      noAccount: 'ليس لديك حساب؟',
      signupLink: 'إنشاء حساب',
      hasAccount: 'لديك حساب بالفعل؟',
      loginLink: 'تسجيل الدخول',
      fullName: 'الاسم الكامل',
      username: 'اسم المستخدم',
      createAccount: 'إنشاء حساب',
      loginSuccess: 'تم تسجيل الدخول بنجاح',
      loginSuccessText: 'مرحباً بك!',
      signupSuccess: 'تم إنشاء الحساب بنجاح',
      signupSuccessText: 'مرحباً! يمكنك الآن تسجيل الدخول.',
      logout: 'تسجيل الخروج',
    },
    account: {
      orders: 'الطلبات',
      addresses: 'العناوين',
      profile: 'الملف الشخصي',
      noOrders: 'لم تقم بأي طلبات بعد',
      noOrdersText: 'عند إتمام طلب، سيظهر هنا.',
      orderNumber: 'طلب',
      statusPending: 'بانتظار تأكيد الدفع',
      statusConfirmed: 'تم التأكيد',
      statusShipped: 'تم الشحن',
      statusDelivered: 'تم التسليم',
      statusWaitingApproval: 'بانتظار الموافقة',
      statusInProcess: 'قيد المعالجة',
      statusInDelivery: 'في الطريق',
      statusNotApproved: 'لم تتم الموافقة',
      statusCancelled: 'ملغي',
      savedAddresses: 'العناوين المحفوظة',
      addAddress: 'إضافة عنوان',
      noAddresses: 'لا توجد عناوين محفوظة',
      defaultAddress: 'افتراضي',
      edit: 'تعديل',
      delete: 'حذف',
      street: 'شارع',
      houseNumber: 'رقم المبنى',
      saveAddress: 'حفظ العنوان',
      cancel: 'إلغاء',
      personalInfo: 'المعلومات الشخصية',
      saveChanges: 'حفظ التغييرات',
      saved: 'تم الحفظ',
      saveError: 'خطأ في الحفظ، حاول مرة أخرى',
      changePassword: 'تغيير كلمة المرور',
      newPassword: 'كلمة مرور جديدة',
      confirmPassword: 'تأكيد كلمة المرور',
      updatePassword: 'تحديث كلمة المرور',
      passwordRequired: 'كلمة المرور مطلوبة',
      passwordTooShort: '6 أحرف على الأقل',
      passwordMismatch: 'كلمتا المرور غير متطابقتين',
      selectAddress: 'اختر عنوان',
      cityRequired: 'المدينة مطلوبة',
      streetRequired: 'الشارع مطلوب',
      houseNumberRequired: 'رقم المبنى مطلوب',
      username: 'اسم المستخدم',
      setDefault: 'تعيين كافتراضي',
      orderDetails: 'تفاصيل الطلب',
    },
    about: {
      heroTitle: 'هندسة خارجية.\nبمستوى آخر.',
      heroSubtitle: 'من نحن',
      heroDescription: 'نصمم ونصنع ونركب برجولات وأنظمة تظليل متقدمة منذ 2008.',
      missionTitle: 'نؤمن بأن مساحتكم الخارجية تستحق نفس مستوى الجودة كالمنزل نفسه.',
      storyTitle: 'قصتنا',
      storyText: 'تأسست AMG Pergola من شغف بالهندسة الدقيقة والتصميم بدون تنازلات. منذ 2008، نقود سوق البرجولات في إسرائيل.',
      storyText2: 'كل برجولة نصنعها تمر بتصميم دقيق وتصنيع محكم وتركيب احترافي.',
      valuesTitle: 'قيمنا',
      values: [
        { title: 'هندسة دقيقة', number: '01', description: 'كل برجولة مصممة بدقة متناهية بمواد مختارة للمتانة.' },
        { title: 'ابتكار', number: '02', description: 'أنظمة ذكية ومحركات هادئة وتحكم عن بعد.' },
        { title: 'خدمة احترافية', number: '03', description: 'فريق متخصص يرافقكم من الاستشارة حتى التركيب.' },
      ],
      ctaTitle: 'مستعدون للبدء؟',
      ctaText: 'تواصل معنا للاستشارة المجانية.',
      ctaButton: 'تحدث معنا',
    },
    contact: {
      title: 'اتصل بنا',
      subtitle: 'نسعد بسماعكم.',
      name: 'الاسم الكامل',
      email: 'البريد الإلكتروني',
      phone: 'الهاتف',
      message: 'الرسالة',
      messagePlaceholder: 'أخبرنا عن مشروعكم...',
      send: 'إرسال الرسالة',
      sending: 'جاري الإرسال...',
      infoTitle: 'معلومات الاتصال',
      emailLabel: 'البريد الإلكتروني',
      phoneLabel: 'الهاتف',
      addressLabel: 'العنوان',
      addressText: 'عتسمونا 1، معاليه أدوميم',
      hoursLabel: 'ساعات العمل',
      hoursText: 'أحد-خميس 08:00-17:00',
    },
    shop: {
      title: 'جميع المنتجات',
      sortBy: 'ترتيب حسب',
      sortOptions: [
        { value: 'featured', label: 'مميز' },
        { value: 'newest', label: 'الأحدث' },
        { value: 'price-asc', label: 'السعر: من الأقل' },
        { value: 'price-desc', label: 'السعر: من الأعلى' },
        { value: 'name-asc', label: 'أبجدي' },
      ],
      filterAll: 'الكل',
      needHelp: 'تحتاج مساعدة؟',
      helpText: 'نحن هنا من أجلك',
      filters: {
        title: 'تصفية',
        searchProducts: 'بحث المنتجات',
        searchPlaceholder: 'ابحث بالاسم أو رقم القطعة...',
        category: 'الفئة',
        subCategory: 'الفئة الفرعية',
        brand: 'العلامة التجارية',
        priceRange: 'السعر',
        priceMin: '₪ الحد الأدنى',
        priceMax: '₪ الحد الأقصى',
        length: 'الطول',
        color: 'اللون',
        sku: 'رقم القطعة',
        skuPlaceholder: 'ابحث برقم القطعة...',
        sort: 'ترتيب',
        newest: 'الأحدث',
        clearAll: 'مسح الكل',
        showResults: 'عرض {count} منتجات',
        categories: 'الفئات',
        meter: 'متر',
      },
    },
    contractor: {
      sku: 'رقم القطعة',
      length: 'الطول',
      size: 'الطول',
      color: 'اللون',
      selectedColor: 'اللون المحدد',
      selectSize: 'يرجى اختيار الطول',
      selectColor: 'يرجى اختيار اللون',
      customColor: 'اختر لون مخصص',
      customColorTitle: 'اختر لون مخصص',
      searchColor: 'ابحث بالاسم أو كود RAL...',
      cancel: 'إلغاء',
      confirmSelection: 'تأكيد الاختيار',
      meter: 'متر',
      addedToCart: 'تمت الإضافة للسلة',
      colorTabs: {
        ral: 'RAL كود اللون',
        passivation: 'تخميل',
        noColorMF: 'بدون لون MF',
        woodLook: 'شبيه الخشب',
        iron: 'حديد',
        metal: 'معدن',
      },
    },
    miniCart: {
      empty: 'سلتك فارغة',
      continueShopping: 'متابعة التسوق',
      freeShipping: 'أنت مؤهل للشحن المجاني.',
      remove: 'إزالة',
      buyWith: 'اشترِ مع',
      add: 'أضف',
      taxNote: 'الضرائب والشحن تُحسب عند الدفع',
      viewCart: 'عرض السلة',
    },
    search: {
      placeholder: 'بحث...',
      clear: 'مسح',
      featuredCollections: 'مجموعات مميزة',
      noResults: 'لم يتم العثور على نتائج.',
      tabs: {
        products: 'منتجات',
        suggestions: 'اقتراحات',
        collections: 'مجموعات',
        blog: 'مدونة',
      },
    },
    pergolaRequest: {
      pageTitle: 'طلب عرسية (بيرغولا)',
      pageSubtitle: 'املأ التفاصيل واحصل على عرض سعر مخصص',
      dimensions: 'الأبعاد',
      width: 'العرض (سم)',
      length: 'الطول (سم)',
      height: 'الارتفاع (سم) - اختياري',
      configuration: 'الإعدادات',
      pergolaType: 'نوع العرسية',
      pergolaTypes: {
        fixed: 'عرسية ثابتة',
        pvc: 'عرسية PVC',
      },
      mountType: 'نوع التركيب',
      mountWall: 'مثبتة على الحائط',
      mountFreestanding: 'مستقلة',
      installation: 'تركيب احترافي',
      lighting: 'إضاءة',
      lightingNone: 'بدون',
      lightingWhite: 'أبيض',
      lightingRgb: 'RGB',
      lightingPosition: 'موقع الإضاءة',
      lightingAllPosts: 'جميع الأعمدة',
      lightingSelectedPosts: 'أعمدة مختارة',
      lightingNoPosts: 'بدون أعمدة',
      lightingFixture: 'نوع وحدة الإضاءة',
      fixtureSpotlight: 'سبوتلايت',
      fixtureLedStrip: 'شريط LED',
      fixtureRgbStrip: 'شريط RGB',
      fixtureMixed: 'حزمة مختلطة',
      lightingRoof: 'إضاءة السقف / المحيط',
      lightingLabel: 'الإضاءة',
      santafRoofing: 'سقف سنطاف',
      santafWithout: 'بدون سنطاف',
      santafWith: 'مع سنطاف',
      santafColorLabel: 'لون السنطاف',
      colors: 'الألوان',
      frameColor: 'لون الإطار',
      roofColor: 'لون السقف / القماش',
      spacing: 'المسافة بين قوارص التقسيم',
      spacingAutomatic: 'تلقائي',
      spacingDense: 'كثيف',
      spacingStandard: 'عادي',
      spacingWide: 'واسع',
      spacingLabel: 'المسافة',
      notes: 'ملاحظات',
      notesPlaceholder: 'أخبرنا عن احتياجاتك...',
      customerInfo: 'معلومات العميل',
      customerName: 'الاسم الكامل',
      customerPhone: 'الهاتف',
      customerEmail: 'البريد الإلكتروني (اختياري)',
      specs: 'المواصفات الفنية',
      moduleType: 'الوحدة',
      moduleSingle: 'فردية',
      moduleDouble: 'مزدوجة',
      moduleTriple: 'ثلاثية',
      moduleCustom: 'يتطلب مراجعة',
      carriers: 'قوارص التقسيم',
      posts: 'الأعمدة',
      frontPosts: 'أعمدة أمامية',
      backPosts: 'أعمدة خلفية',
      profileRafter: 'بروفيل رافتر',
      profileCarrierPost: 'بروفيل عمود قارص تقسيم',
      profileFabricMaster: 'بروفيل قماش رئيسي',
      preview: 'معاينة',
      topView: 'منظر علوي',
      frontView: 'منظر أمامي',
      isometricView: 'ثلاثي الأبعاد',
      submit: 'إرسال الطلب',
      submitting: 'جارٍ الإرسال...',
      submitConfirmTitle: 'تأكيد الإرسال',
      submitConfirmText: 'هل أنت متأكد من إرسال الطلب؟',
      confirm: 'تأكيد',
      cancel: 'إلغاء',
      successTitle: 'تم إرسال الطلب بنجاح!',
      successText: 'سنتواصل معك قريبًا بعرض السعر.',
      backToHome: 'العودة للرئيسية',
      disclaimer: 'هذا ملخص طلب وليس عرض سعر نهائي. الرسم التخطيطي وتوزيع الأعمدة أولية وتخضع للفحص الفني وزيارة الموقع.',
      customReviewNote: 'العرض المدخل يتطلب مراجعة خاصة. سيتواصل معك أحد ممثلينا.',
      required: 'حقل مطلوب',
      invalidPhone: 'رقم هاتف غير صالح',
      minWidth: 'الحد الأدنى للعرض 100 سم',
      maxWidth: 'الحد الأقصى للعرض 1500 سم',
      minLength: 'الحد الأدنى للطول 200 سم',
      maxLength: 'الحد الأقصى للطول 1000 سم',
      yes: 'نعم',
      no: 'لا',
      clickToEdit: 'انقر على عنصر في الرسم للتعديل',
      startSubtitle: 'أدخل الأبعاد الأساسية واختر نوع العرسية للبدء',
      startEditor: 'ابدأ التصميم',
      nextStep: 'التالي',
      backToEditor: 'العودة إلى المحرر',
      summaryTitle: 'الملخص الفني',
      partsTitle: 'المكونات والبروفيلات',
      partsSubtitle: 'المكونات الرئيسية التي تشكل العرسية الخاصة بك',
      roofFillMode: 'وضع تعبئة السقف',
      roofSlats: 'شرائح / بروفيلات',
      roofSantafOnly: 'سنطاف فقط',
      slatsLabel: 'شرائح',
      slatGap: 'المسافة بين الشرائح',
      slatColor: 'لون الشرائح',
      slatSizeLabel: 'حجم بروفيل الشريحة',
      slatsPerCarrier: 'شرائح لكل قارص تقسيم',
      globalApplyHint: 'التغيير هنا ينطبق على جميع قوارص التقسيم. انقر على قارص محدد في الرسم للتخصيص',
      lightingPosts: 'إضاءة الأعمدة',
      lightingLengthLabel: 'طول شريط الإضاءة',
      lightingPerCarrierHint: 'انقر على قارص تقسيم في الرسم لاختيار إضاءة لكل قارص بشكل منفصل',
      phoneMustBe10: 'يجب أن يتكون رقم الهاتف من 10 أرقام',
      downloadPdf: 'تحميل PDF',
      downloadPdfTitle: 'ملخص الطلب جاهز',
      downloadPdfDesc: 'يشمل الرسومات الفنية والمواصفات وتفاصيل الإعدادات',
    },
    notFound: {
      title: 'الصفحة غير موجودة',
      text: 'الصفحة التي تبحث عنها غير موجودة.',
      home: 'العودة للرئيسية',
      shop: 'المتجر',
    },
  },
};